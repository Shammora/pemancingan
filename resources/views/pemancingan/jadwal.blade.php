            ...
            <div class="col-xs-12">
                <div class="box box-danger">
                    <div class="box-header">
                        <h3 class="box-title">Data Jadwal Pemancingan</h3>
                        ...
                        <table class="table table-bordered table-striped" id="data-pemancing">
                            <thead>
                                <tr>
                                    ...
                                    <th>Jam</th>
                                    <th>Tiket</th>
                                    <th style="display: none">Pemancingan ID</th>
                                    <th>Pemancingan</th>
                                    <th>Opsional</th>
                                    <th>Aksi</th>
                                </tr>
                            ...
                            <tbody>
                                @foreach (@$jadwal as $key => $value)
                                    <tr>
                                        ...
                                        <td>{{ @$value->hari }}</td>
                                        <td>{{ @$value->jam }}</td>
                                        <td>Rp.{{ number_format($value->tiket, 0, ',', '.') }}</td>
                                        <td style="display: none">{{ @$value->pemancingan_id }}</td>
                                        <td>{{ @$value->Pemancingan->nama }}</td>
                                        <td>{{ @$value->opsional }}</td>
                                        <td>
                                            <button class="btn btn-xs btn-success btn-edit-pemancing"><i class="fa fa-edit">
                                                    Ubah</i></button> &nbsp;
                                            <a href="{{ route('pemancingan.deleteJadwal', $value->id) }}"><button
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('pemancingan.addJadwal') }}" method="post" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="form-group has-feedback">
                            <label>Pemancingan:</label>
                            <select name="pemancinganId" id="pemancinganId" class="form-control" required>
                                <option value="">Pilih</option>
                                @foreach ($pemancingan as $value)
                        ...
                        <div class="form-group has-feedback">
                            <label>Tiket :</label>
                            <input type="number" name="tiket" class="form-control" placeholder="Harga Tiket" required>
                        </div>
                        <div class="form-group has-feedback">
                            <label>Optional :</label>
                            <textarea name="opsional" id="opsional" class="form-control" cols="4" rows="2" required></textarea>
                        </div>
                        <div class="row">
    ...
    <div class="modal fade" id="modal-form-edit-pemancing" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('pemancingan.updateJadwal') }}" method="post" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <div class="form-group has-feedback">
                        ...
                        <div class="form-group has-feedback">
                            <label>Tiket :</label>
                            <input type="number" name="tiket" class="form-control" placeholder="Harga Tiket"
                                required>
                        </div>
                        <div class="form-group has-feedback">
                            <label>Optional :</label>
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        $('#data-pemancing').on('click', '.btn-edit-pemancing', function() {
            ...
            $('input[name=hari]').val(row[1]);
            $('input[name=jam]').val(row[2])
            $('input[name=tiket]').val(row[3])
            $('select[name=pemancinganId]').val(row[4])
            $('textarea[name=opsional]').val(row[6]);
