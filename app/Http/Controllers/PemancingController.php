...
class PemancingController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $hasilData = 'Data tidak ditemukan'; // Variabel yang tampil jika data tidak ditemukan
        $hasilIndex = null; // Variabel untuk menyimpan index dari data yang berhasil ditemukan
        $status = 'false'; // Status pencarian awal = false
        $prosesPencarian = []; // Variabel untuk menampilkan proses pencarian di halaman pencarian

        // Proses perulangan pada algoritma pencarian sequential
        // Perulangan mulai dari index ke 0 hingga index terakhir dalam array
        for ($i = 0; $i < count($pemancinganArray); $i++) {

            // Isi variabel prosesPencarian dengan data-data pencarian seperti tahap ke, perbandingan index, dan status ditemukan
            $prosesPencarian[] = [
                'cari' => $request->search,
                'tahap' => $i + 1,
            ...
            if (strtolower($pemancinganArray[$i]->nama) == strtolower($request->search)) {
                ...
                $hasilIndex = $i; // Isi hasil index dengan index dari data yang ditemukan
                $status = 'true'; // Status pencarian true, artinya data berhasil ditemukan
                $prosesPencarian[$i]['ditemukan'] = true; // Proses pencarian true, maka proses pencarian akan ditampilkan

                break; // Hentikan pencarian apabila data berhasil ditemukan
            }
        }

        return view('pemancing.search', compact('title', 'dataLengkap', 'pemancinganArray', 'hasilData', 'hasilIndex', 'status', 'prosesPencarian'));
    }
